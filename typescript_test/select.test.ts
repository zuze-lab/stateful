...
describe('select', () => {
  ...
  it('should create a selector factory', () => {
    const always = createSelectorFactory(() => true);
    const spy = jest.fn(a => a);
    const selector = always(spy);
    expect(selector('fred')).toBe('fred');
    expect(selector('joe')).toBe('fred');
    expect(spy).toHaveBeenCalledTimes(1);
    expect(spy).toHaveBeenCalledWith('fred');
  });
});
